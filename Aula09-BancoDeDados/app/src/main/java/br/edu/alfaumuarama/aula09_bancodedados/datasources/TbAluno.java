...
public class TbAluno {
    ...
    public TbAluno(Context context) {
        ...
        String sql = "CREATE TABLE IF NOT EXISTS tbAluno (" +
                     ...
                     " nome TEXT, ra TEXT, cidade TEXT, uf TEXT)";

        //criando a tabela no banco, caso a mesma nao exista
        BancoDados.getInstance().executarSQL(sql);

        //adicionando a nova coluna para os aplicativos que ja utilizavam a versao antiga
        BancoDados.getInstance().adicionarNovaColuna("tbAluno", "uf", "TEXT", "PR");
    }
    ...
    public ArrayList<Aluno> buscar(String filtro) {
        /*
        ...
        */
        Cursor cursor = BancoDados.getInstance().getDb().query(
            "tbAluno", //nome da tabela
            new String[] { "id", "nome", "ra", "cidade", "uf"}, //colunas retornadas
            "cidade = ?", //condicao - filtro - WHERE
            new String[] { filtro }, //parametros da condicao acima
            null, //group by
            null, //having
            "nome", //ORDER BY
            null //LIMIT
    ...
    private ArrayList<Aluno> retornarLista(Cursor cursor) {
        ...
        if (cursor.getCount() > 0) { //verifica se existe dados no cursor
            ...
            int indiceCampoUf = cursor.getColumnIndex("uf");
            ...
            for (int i = 0; i < cursor.getCount(); i++) {
                ...
                aluno.uf = cursor.getString(indiceCampoUf);
